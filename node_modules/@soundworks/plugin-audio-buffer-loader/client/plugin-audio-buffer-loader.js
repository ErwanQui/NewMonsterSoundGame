...
const pluginFactory = function (AbstractPlugin) {
  ...
  return class PluginAudioBufferLoader extends AbstractPlugin {
    constructor(client, name, options) {
      ...
      const defaults = {
        assetsDomain: '',
        data: null // directories:

      };
      this.definitions = {};
      this.data = {};
      this.options = this.configure(defaults, options);
      this._loader = new _wavesLoaders.SuperLoader();
    }
    ...
    async load(defObj, override = false) {
      ...
      const dataObj = (0, _lodash.default)(defObj); // decompose def given object

      decomposePathObj(dataObj, pathList, refList, false);
      const prefixedPathList = prefixPaths(pathList, this.options.assetsDomain); // allow special caracters in filename (ex: sound-A#.mp3)
      ...
      if (prefixedURI.length) {
        try {
          ...
          populateRefList(refList, loadedObjList); // mix into this data
